...
def draw_premium_jieba_logo(output_file='jieba_logo_enhanced.png'):
    ...
    def get_pentagon_verts(center, size, rotation_deg):
        ...
        return np.column_stack([center[0] + size * np.cos(angles), center[1] + size * 0.85 * np.sin(angles)])
    ...
    for i, step in enumerate(steps):
        ...
        shadow = patches.Polygon(shadow_verts, closed=True, color='black', alpha=0.05, zorder=1)
        ax.add_patch(shadow)
